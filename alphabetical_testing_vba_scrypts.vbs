Sub alphabetical_Macro2Sort()
'
' Keyboard Shortcut: Ctrl+Shift+A
'
Dim ws As Worksheet
Dim WS_sort_Count As Integer
Dim WS_sort_I As Integer
Dim last_sort_row As Integer

last_sort_row = Cells(Rows.Count, 1).End(xlUp).Row
'
' Set WS_sort_Count equal to the number of worksheets in the active workbook.
WS_sort_Count = ActiveWorkbook.Worksheets.Count

' Begin the loop.
For WS_sort_I = 1 To WS_sort_Count
ActiveWorkbook.Worksheets(WS_sort_I).Select
Columns("A:G").Select
ActiveWorkbook.Worksheets(WS_sort_I).Sort.SortFields.Clear
ActiveWorkbook.Worksheets(WS_sort_I).Sort.SortFields.Add2 Key:=Range((Cells(1, 1).Address & ":" & Cells(last_sort_row, 1).Address)), _
SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
With ActiveWorkbook.Worksheets(WS_sort_I).Sort
    .SetRange Range(Cells(1, 1).Address & ":" & Cells(last_sort_row, 7).Address)
    .Header = xlYes
    .MatchCase = False
